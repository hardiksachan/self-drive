# Source: https://github.com/vadim7s/SelfDrive/blob/master/Semantic%20segmentation/generate%20images.py
...
def main():
    ...
    try:
        ...
        camera_bp.set_attribute("image_size_x", CAM_IM_WIDTH)
        camera_bp.set_attribute("image_size_y", CAM_IM_HEIGHT)
        ...
        camera_bp.set_attribute(
            "image_size_x", CAM_IM_WIDTH
        )  # this ratio works in CARLA 9.13 on Windows
        camera_bp.set_attribute("image_size_y", CAM_IM_HEIGHT)
        camera_bp.set_attribute("fov", CAM_FOV)
        camera_init_trans = carla.Transform(
            carla.Location(z=CAMERA_POS_Z, x=CAMERA_POS_X)
        ...
        while img_counter < IMG_COUNT:
            ...
            distance_progress = current_position.location.distance(
            ...
            ):  # previous time only updated when car moved at least 0.1 meter
                previous_time = current_time

            if time_at_same_location > 120:  # this is 'stuck in traffic' situation
            ...
            if distance_progress > 5:
                ...
                cv2.imwrite(
                    os.path.join(
                        project_workspace, "out_sem", "rgb", "%06d.png" % time_grab
                    ),
                    rgb_im,
                )
                cv2.imwrite(
                    os.path.join(
                        project_workspace, "out_sem", "sem", "%06d.png" % time_grab
                    ),
                    sem_im,
                )
                previous_position = hero_car.get_transform()
