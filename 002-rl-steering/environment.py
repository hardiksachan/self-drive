...
class CarEnv(gym.Env):
	...
	def __init__(self):
		...
		self.observation_space = spaces.Box(low=0.0, high=1.0,
                                            shape=(512,4,9), dtype=np.float32)
	...
	def step(self, action):
		...
		if self.episode_start + SECONDS_PER_EPISODE < time.time():
			...
			print("Successful...")
			self.cleanup()
		self.image = self.front_camera[self.height_from:,self.width_from:self.width_to]
		self.features = self.extract_features(self.image)

		return self.features, reward, done, {}	#curly brackets - empty dictionary required by SB3 format
	...
	def extract_features(self, image):
		...
		features = self.resnet34(tensor.unsqueeze(0))
