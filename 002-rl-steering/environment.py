...
class CarEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(
            low=0.0, high=1.0, shape=(1024, 4, 9), dtype=np.float32
        )
        ...
        # Load the pretrained ResNet34 model and set it to evaluation mode
        self.shufflenet = models.shufflenet_v2_x1_0(pretrained=True)
        self.shufflenet = torch.nn.Sequential(*list(self.shufflenet.children())[:-1])  # remove the last layer
    ...
    def step(self, action):
        ...
        # optional - print steer and throttle every 10 steps
        ...
        if self.episode_start + SECONDS_PER_EPISODE < time.time():
            ...
            print("Successful...")
            self.cleanup()
        self.image = self.front_camera[
            self.height_from :, self.width_from : self.width_to
        ]
        ...
        return (
            ...
            {},
        )  # curly brackets - empty dictionary required by SB3 format
    ...
    def reset(self):
        ...
        self.sem_cam = self.blueprint_library.find(
        ...
        self.lanesensor = self.world.spawn_actor(
            lanesensor, camera_init_trans, attach_to=self.vehicle
        )
    ...
    def process_img(self, image):
        ...
        i = i.reshape((self.im_height, self.im_width, 4))[
            :, :, :3
        ]  # this is to ignore the 4th Alpha channel - up to 3
        self.front_camera = i
    ...
    def extract_features(self, image):
        ...
        # Add an extra dimension to the tensor and pass it through the ResNet34 model
        features = self.shufflenet(tensor.unsqueeze(0))

        # Remove the extra dimension and convert the tensor back to a numpy array
